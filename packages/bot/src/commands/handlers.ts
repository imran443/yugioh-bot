...
export function tournamentSignupPostReply(input: {
  ...
  const webLink = input.webSlug ? `\nManage on web: ${WEB_URL}/tournament/${input.webSlug}` : "";

  return {
    content: `${prefix}Signups are open for ${input.name} (${input.format}).${seededText} Click Join Tournament to enter.${webLink}`,
    components: [
      new ActionRowBuilder<ButtonBuilder>().addComponents(
        new ButtonBuilder()
...
export function draftSignupPostReply(input: { id: number; name: string; webSlug?: string }): CommandReplyLike {
  const webLink = input.webSlug ? `\nPick cards here: ${WEB_URL}/draft/${input.webSlug}` : "";
  return {
    content: `Signups are open for ${input.name}. Click Join Draft to enter.${webLink}`,
    components: [
      new ActionRowBuilder<ButtonBuilder>().addComponents(
        new ButtonBuilder()
...
async function handleEvent(
  ...
  switch (subcommand) {
    ...
    case "dashboard": {
      await interaction.reply({
        ...tournamentDashboardReply(),
        content: `${(tournamentDashboardReply() as { content: string }).content}\nOpen web dashboard: ${WEB_URL}/dashboard`,
      });
      return;
    }
    case "signup": {
