...
describe("DraftCardPreview", () => {
  ...
  it("renders the full-resolution image without card detail text", () => {
    ...
    expect(preview).toHaveClass("w-[calc(14rem+max(0px,(100vw-114rem)/2)-1rem)]");
    expect(preview).toHaveClass("min-w-[14rem]");
    expect(preview).toHaveClass("max-w-[36rem]");
    expect(art).toHaveClass("aspect-[421/614]");
    expect(image).toHaveAttribute("src", "https://img/full/101");
    expect(image).toHaveAttribute("sizes", "(min-width: 1280px) 36rem, 0px");
