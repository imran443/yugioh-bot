...
export function ThemeEditor({ themeId }: { themeId: number }) {
  ...
  const applyDetail = React.useCallback(
    (data: { theme?: ThemeDto; pools: ThemePoolsDto; cards: CardSummary[] }) => {
  ...
  const mutate = async (op: Record<string, unknown>) => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/themes/${themeId}/cards`, {
        method: "POST",
  ...
  if (loading) return <p className="text-sm text-text-secondary">Loading theme...</p>;
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-xl border border-border bg-surface p-4">
        ...
        <div className="relative">
          <label htmlFor="theme-card-search" className="mb-1 block text-sm font-medium text-text-primary">Search card name</label>
          <input
            id="theme-card-search"
      ...
      <section className="rounded-xl border border-border bg-surface p-4">
        ...
        <label htmlFor="theme-import-file" className="mb-1 block text-sm font-medium text-text-primary">Upload text file</label>
        <input
        ...
        />
        <label htmlFor="theme-import-text" className="mb-1 block text-sm font-medium text-text-primary">Paste passcodes</label>
        <textarea
        ...
        <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-xs text-text-secondary">Imported cards are added to the theme, routed to main/extra automatically.</p>
