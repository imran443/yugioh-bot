...
- *Pool/library ops — renamed 1:1 from the themes service:* `createBlank`, `createFromArchetype`, `findCube` (was `findTheme`), `listCubes` (was `listThemes`), `getCubePools` (was `getThemePools`), `addCard`, `removeCard`, `setMaxCopies`, `importPasscodes`, `seedArchetypeInto`, `analyzeCubePools` (was `analyzeTheme`, already exists at `themes.ts:240`).
- *Pool/library ops — NEW methods (not present on the themes service today):* `renameCube` and `deleteCube` centralize what is currently inline SQL in `packages/web/app/api/themes/[id]/route.ts` (`update themes set name…` / `delete from themes…`). The engine's `preflightThemes` currently duplicates the `analyzeCubePools` sufficiency math inline; it is changed to delegate to `analyzeCubePools` so there is one analyzer. Budget `renameCube`/`deleteCube` as new code; the rest are renames.
